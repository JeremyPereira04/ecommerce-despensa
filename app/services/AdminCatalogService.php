...
final class AdminCatalogService
{
    ...
    public function publicidades():array{return $this->repo->publicidades();}
    public function publicidad(int $id):?array{return $this->repo->publicidad($id);}
    public function guardarPublicidad(array $input,?string $image):int
    {
        if($image===null){throw new InvalidArgumentException('Seleccioná una imagen publicitaria para el carrusel.');}
        ...
        $order=filter_var($input['orden']??0,FILTER_VALIDATE_INT);
        if($order===false||$order<0||$order>999){throw new InvalidArgumentException('El orden debe ser un número entre 0 y 999.');}
        return $this->repo->guardarPublicidad(['imagen'=>$image,'texto_alternativo'=>$alt,'orden'=>$order,'activo'=>isset($input['activo'])?1:0]);
    }
    public function cambiarPublicidad(int $id):void{$this->repo->cambiarPublicidad($id);}
    public function eliminarPublicidad(int $id):?array{return $this->repo->eliminarPublicidad($id);}
